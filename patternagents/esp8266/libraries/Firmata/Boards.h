...
/* ThingSoC ESP8266 Wi-Fi Board
 TSOC_ESP8266 adds a 16 bit I/O Expander to the Generic ESP8266
 ESP8266 GPIO Pins (default) are as follows:
GPIO   : Pin Function
GPIO0  : SLOT_RXD
GPIO1  : USB_RXD        (Do Not Use in Firmata)
GPIO2  : SLOT_TXD
GPIO3  : USB_TXD        (Do Not Use in Firmata)
GPIO4  : SDA            (Do Not Use in Firmata)
GPIO5  : SCL            (Do Not Use in Firmata)
GPIO6-GPIO11 : Internal (Do Not Use in Firmata)
GPIO12 : MISO 
GPIO13 : MOSI 
GPIO14 : SCK
GPIO15 : SS
GPIO16 :       Internal (Do Not Use in Firmata)
GPIO17 : A0    (Single Analog Input)
 I2C I/O Expander GPIO Pins (default) are as follows:
GPIO18 : SLOT_CTS
GPIO19 : SLOT_RTS
GPIO20 : SLOT_DTR
GPIO21 : SLOT_DSR
...
GPIO28 : SLOT_IRQ
GPIO29 : SLOT_ATN 
GPIO30 : SLOT_RED (BUILTIN_LED)
GPIO31 : SLOT_GREEN
GPIO32 : SLOT_BLUE 
GPIO33 : SLOT_EN
*/
#elif defined(TSOC_ESP8266)
#define TOTAL_ANALOG_PINS       1
#define TOTAL_PINS              34 /* 33 digital + 1 analog */
#define VERSION_BLINK_PIN       31
#define IS_PIN_DIGITAL(p)       ((p) == 0 || (p) == 2 || ((p) >= 12 && (p) <= 15) || ((p) >= 18 && (p) <= 33))
#define IS_PIN_ANALOG(p)        ((p) == 17)
#define IS_PIN_PWM(p)           ((p) == 0 || (p) == 2 || ((p) >= 12 && (p) <= 15) || ((p) >= 18 && (p) <= 33))
#define IS_PIN_SERVO(p)         ((p) == 0 || (p) == 2 || ((p) >= 12 && (p) <= 15) || ((p) >= 18 && (p) <= 33))
#define IS_PIN_I2C(p)           (0) /* I2C is used for the I/O Expander (D18-D33), don't trash it's operation... */
#define IS_PIN_SPI(p)           ((p) == SS || (p) == MOSI || (p) == MISO || (p) == SCK)
#define PIN_TO_DIGITAL(p)       (p)
